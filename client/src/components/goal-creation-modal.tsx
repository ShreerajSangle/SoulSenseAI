...
export function GoalCreationModal({ isOpen, onClose, onGoalCreated, persona }: GoalCreationModalProps) {
  ...
  const suggestions = GOAL_SUGGESTIONS[persona.id] || GOAL_SUGGESTIONS.sarah;
  const encouragement = PERSONA_ENCOURAGEMENT[persona.id] || PERSONA_ENCOURAGEMENT.sarah;
  ...
  return (
    <Card className="w-full bg-white/95 backdrop-blur-sm border border-purple-200/50 shadow-xl animate-in zoom-in-95 duration-200">
      <CardHeader className="pb-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="font-medium text-gray-800">Set a Goal with {persona.name}</h3>
          </div>
          <Button variant="ghost" size="sm" onClick={onClose} className="hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
            <X className="h-4 w-4" />
          </Button>
        </div>
      ...
      <CardContent className="space-y-4">
        ...
        <div className="flex gap-2 pt-2">
          ...
          <Button
            ...
            className="flex-1 bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 text-white transition-all hover:scale-105 disabled:opacity-50 disabled:scale-100"
          >
